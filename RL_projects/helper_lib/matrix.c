...
cl_int CheckMatrix(Matrix *truth, Matrix *student)
{
    ...
    {
        float EPSILON = (float) fabs(truth->data[i])*0.5;//  +/- 5% tolerance
        if (EPSILON < 1e-7f)
            continue;      
        float diff = (float) fabs(truth->data[i] - student->data[i]);
        if ( diff> EPSILON && diff>0.19) {
            printf("difference = %f, EPSILON=%f\n", diff, EPSILON);
            printf("!!SOLUTION IS NOT CORRECT!!, Element[%d] S / B = %f Vs Students = %f\n", i, truth->data[i], student->data[i]);
            return CL_INVALID_VALUE;
        }
    }
